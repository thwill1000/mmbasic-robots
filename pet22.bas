      ...
      select case UT(i)
        ...
        case 2,3
          if UH(i)>0 then
            ...
            Sprite compressed sprite_index(&h49),xs+24*dx,ys+24*dy,9  'dead bot
          end if
        case 9
          'temp choice for sprite
          if UH(i)>0 then
            Sprite compressed sprite_index(&h39),xs+24*dx,ys+24*dy,9
          else
            Sprite compressed sprite_index(&h4B),xs+24*dx,ys+24*dy,9  'dead bot
          end if
        case 11 'small explosion
          Sprite compressed tile_index(UA(i)),xs+24*dx,ys+24*dy,0
        case 12 'shoot up
        ...
        case 17,18
          if UH(i)>0 then
            ...
            Sprite compressed sprite_index(&h4A),xs+24*dx,ys+24*dy,9  'dead bot
          end if
        ...
        case 71,72 'bomb/magnet visible
          if UB(i)>0 then Sprite compressed sprite_index(UA(i)),xs+24*dx,ys+24*dy,9
      end select
    end if
  next i
  framebuffer write n
  ...
  For i=0 To 47                       'unit 0 = player, skip player
    ...
    Select Case UT(i)
      ...
      Case 10 'automatic doors
        ...
        EndIf
      Case 11 'small explosion
        inc UA(i),1:if UA(i)=252 then UT(i)=0 'done exploding
      ...
      case 71 'bomb
        inc UB(i),-1
        if UB(i)=0 then
          do_damage(UX(i),UY(i),5,11) 'do damage 11 in radius 5 to units
          'damage_tiles(UX(i),UY(i),5,11) 'do damage 11 in radius 5 to tiles
          Play modsample s_dsbarexp,4
        end if
        if UB(i)<0 then xof=4-xof:xs=5*24 - xof 'shake screen
        if UB(i)=-10 then xs=5*24:UT(i)=0       'free slot, screen centred
      end if
    case 72 'magnet
      inc UB(i),-1
      if UB(i)<0 then
...
sub place_bomb
'UA()=sprite,UB()=delay in loops (around 3 seconds)
local i=findslot()
if i<32 then
  UT(i)=71:UX(i)=xp+h:UY(i)=yp+v:UA(i)=&h57:UB(i)=24
...
Sub fire_ew(p)
'UD() is the tile of the target (or 0)
  ...
  if i<32 then
    ...
    Do
      ...
      if j then' if robot then damage it
        ...
        if d=&h83 then mid$(lv$(yp),xp+x+1,1)=chr$(&h87)  'canister
        inc x,-p:Exit do
  ...
  if i<32 then
    ...
    Do
      ...
      if j then 'if robot then damage it
        ...
        if d=&h83 then mid$(lv$(yp+y),xp+1,1)=chr$(&h87)   'canister
        inc y,-p:Exit do
