...
Sub show_mode(tile)
  FRAMEBUFFER write l
  Static x
  If x=0 Then Sprite compressed sprite_index(tile),xs,ys,9  'over player
...
Sub anim_viewer
  ...
  Select Case p
    Case 4,8
      ...
      Sprite compressed sprite_index(&h53),h2+8,v2+8,9
    Case 3,7
      Box h2-8,v2-8,24,24,1,col%(5),col%(5)
      Sprite compressed sprite_index(&h53),h2+8,v2-8,9
    Case 2,6
...
Sub target_move
  ...
  If (get_ta(ox,oy) And b_mov) = b_mov Then     'can object be moved?
    FRAMEBUFFER write l
    Sprite compressed sprite_index(&h55),24*h+xs,24*v+ys,9  'show hand
    FRAMEBUFFER write n
  Else
