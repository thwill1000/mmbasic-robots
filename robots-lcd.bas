  ...
  Do
    ...
    If ky=9 Then 'TAB key show map + toggle player/robots
      Select Case map_mode
        ...
        Case 2
          map_mode=0
          FRAMEBUFFER write l:CLS  col%(5):FRAMEBUFFER write f 'clear layer
...
Sub show_mode(tile)
  FRAMEBUFFER write l
  Static x
  If x=0 Then Blit compressed sprite_index(tile),xs,ys,9  'over player
  If x=1 Then writeplayer_m(0,0,pl_wp)                    'restore player
  'x=2 and x=3 do nothing
  x=(x+1) And 3
  FRAMEBUFFER write f
End Sub
...
Sub anim_viewer
  ...
  Select Case p
    Case 4,8
      Box h2+8,v2-8,24,24,1,col%(5),col%(5)
      Blit compressed sprite_index(&h53),h2+8,v2+8,9
    Case 3,7
      Box h2-8,v2-8,24,24,1,col%(5),col%(5)
      Blit compressed sprite_index(&h53),h2+8,v2-8,9
    Case 2,6
    ...
    Case 1,5
      ...
      Blit compressed sprite_index(&h53),h2-8,v2+8,9
    Case 9
      p=0:Box h2+8,v2+8,24,24,1,col%(5),col%(5)
  End Select
  FRAMEBUFFER write f
...
Sub target_move
  ...
  writeplayer_m(h,v,pl_wp)                    'remove old hand
  If (get_ta(ox,oy) And b_mov) = b_mov Then     'can object be moved?
    FRAMEBUFFER write l
    Blit compressed sprite_index(&h55),24*h+xs,24*v+ys,9  'show hand
    FRAMEBUFFER write f
  Else
...
Sub show_keys
  ...
  For i=0 To 2
    If pl_ky And 1<<i Then
      Blit compressed key_index(i),271+16*i,124
    EndIf
  Next
End Sub
...
Sub show_weapon
  If pl_wp>0 Then
    Blit compressed item_index(pl_wp-1),272,38
...
Sub show_item
  If pl_it>0 Then
    Local a$,b$
    Blit compressed item_index(pl_it+1),272,81
    Select Case pl_it
      Case 1'medkit
        a$="medkit"
        b$=Str$(pl_mk,3,0)
...
Sub update_player
  ...
  If oldhealth<>UH(0) Then
    Blit compressed health_index(Int((12-UH(0))/2)),272,160
    oldhealth=UH(0)
    For i=1 To 12
      If i > oldhealth Then
...
Sub writeplayer_m(h,v,w)
  ...
  FRAMEBUFFER write l
  CLS  col%(5) 'erase whole layer L
  pl_sp=8*(v=-1)+4*(h=1)+12*(h=-1)      'sprite matching orientation
  Box xs,ys,24,24,,col%(5),col%(5)
  Blit compressed sprite_index(pl_sp+pl_mv+16*w),xs,ys,9
...
Sub writeworld_m(xm,ym)
  For xn=-xm To xm
    For yn=-ym To ym
      ...
      Blit compressed tile_index(spn),x,y
    Next
  Next
End Sub
...
Sub walk_bot_h(i,dx,dy,hov)
  Local xy
  Box xs+24*dx,ys+24*dy,24,24,1,col%(5),col%(5) 'erase earlier position
  UD(i)=(UD(i)+1) Mod 3
  If UD(i)=0 Then
    ...
    Else
      ...
      'UX(i)=UX(i)+2*(UC(i))-1   'go back
    EndIf
    dx=UX(i)-xp:dy=UY(i)-yp
  EndIf
  If Abs(dx)<=xm And Abs(dy)<=ym Then
    Blit compressed sprite_index(&h31+(hov=0)*4),xs+24*dx,ys+24*dy,9
  EndIf
End Sub
    ...
    If nearx<=xm And neary<=ym Then   'withing viewing window
      Select Case UT(i)
        ...
        Case 9 'evilbot chase player
          Blit compressed sprite_index(&h39),xs+24*dx,ys+24*dy
