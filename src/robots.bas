...
' Gets the path to a file/asset. First tries relative to the location of this
' file and if not found there tries relative to this files parent.
Function path$(f$)
  Local i%
  For i% = 1 To 2
    ...
    End Select
    Cat path$, "/"
    If i% = 2 Then Cat path$, "../"
    Cat path$, f$
    If Mm.Info(Exists File path$) Then Exit For
  Next
End Function
